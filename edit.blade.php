...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-extrabold text-slate-800">Ubah Kehadiran: {{ $kelas->nama_mata_kuliah }}</h1>
                <p class="text-xs text-slate-400 font-medium">Ubah lembar kehadiran mahasiswa untuk Ruang: {{ $kelas->ruangan }}.</p>
            </div>
        </div>
    </div>

    <form action="{{ route('presensi.update', $tanggal) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="space-y-1.5 w-full md:w-72">
                <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Tanggal Presensi</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400">
                        <i class="fa-solid fa-calendar-day"></i>
                    </span>
                    <input type="date" disabled value="{{ $tanggal }}" class="w-full pl-10 pr-4 py-2 bg-slate-100 border border-slate-200 rounded-xl text-sm text-slate-500 cursor-not-allowed">
                        ...
                        @foreach($mahasiswas as $index => $mhs)
                            ...
                            <tr class="hover:bg-slate-50/30 transition-colors group">
                                ...
                                <td class="py-4 px-6">
                                    ...
                                    @if($presensi && $presensi->latitude && $presensi->longitude)
                                        <div class="text-[10px] text-indigo-650 font-bold mt-1 space-y-0.5 bg-indigo-50/50 p-1.5 rounded-xl border border-indigo-100/40 w-fit flex flex-col gap-0.5">
                                            <span class="flex items-center gap-1 text-[9px] text-slate-500">
                                                <i class="fa-solid fa-location-dot text-[10px]"></i>
                                                <span>GPS: {{ round($presensi->latitude, 5) }}, {{ round($presensi->longitude, 5) }}</span>
                                            </span>
                                            <span class="flex items-center gap-1 text-[9px] text-slate-500">
                                                <i class="fa-solid fa-route text-[10px]"></i>
                                                <span>Jarak: {{ round($presensi->jarak, 1) }} meter</span>
                                            </span>
                                        </div>
                                    @endif
                                </td>
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        <label class="relative flex items-center justify-center px-4 py-2 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 select-none transition-all has-[:checked]:bg-emerald-500 has-[:checked]:border-emerald-500 has-[:checked]:text-white has-[:checked]:shadow-sm">
                                            <input type="radio" name="presensi[{{ $mhs->id }}][status]" value="Hadir" class="sr-only" {{ $status == 'Hadir' ? 'checked' : '' }} onclick="toggleKeterangan({{ $mhs->id }}, false)">
                                        ...
                                        <label class="relative flex items-center justify-center px-4 py-2 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 select-none transition-all has-[:checked]:bg-blue-500 has-[:checked]:border-blue-500 has-[:checked]:text-white has-[:checked]:shadow-sm">
                                            <input type="radio" name="presensi[{{ $mhs->id }}][status]" value="Sakit" class="sr-only" {{ $status == 'Sakit' ? 'checked' : '' }} onclick="toggleKeterangan({{ $mhs->id }}, true)">
                                        ...
                                        <label class="relative flex items-center justify-center px-4 py-2 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 select-none transition-all has-[:checked]:bg-amber-500 has-[:checked]:border-amber-500 has-[:checked]:text-white has-[:checked]:shadow-sm">
                                            <input type="radio" name="presensi[{{ $mhs->id }}][status]" value="Izin" class="sr-only" {{ $status == 'Izin' ? 'checked' : '' }} onclick="toggleKeterangan({{ $mhs->id }}, true)">
                                        ...
                                        <label class="relative flex items-center justify-center px-4 py-2 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 select-none transition-all has-[:checked]:bg-rose-500 has-[:checked]:border-rose-500 has-[:checked]:text-white has-[:checked]:shadow-sm">
                                            <input type="radio" name="presensi[{{ $mhs->id }}][status]" value="Alpa" class="sr-only" {{ $status == 'Alpa' ? 'checked' : '' }} onclick="toggleKeterangan({{ $mhs->id }}, false)">
                                            <span class="text-xs font-bold">Alpa</span>
                                        </label>
                                    </div>
                                </td>
                                <td class="py-4 px-6">
                                    <input type="text" name="presensi[{{ $mhs->id }}][keterangan]" id="keterangan-{{ $mhs->id }}" 
                                        {{ $isKetEnabled ? '' : 'disabled' }} 
                                        value="{{ $keterangan }}"
                                        placeholder="{{ $isKetEnabled ? 'Tulis alasan sakit/izin...' : 'Hadir tidak perlu keterangan' }}" 
                                        class="w-full px-3 py-1.5 {{ $isKetEnabled ? 'bg-white' : 'bg-slate-100' }} border border-slate-200 rounded-lg text-xs focus:outline-none focus:border-brand-500 focus:bg-white transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
